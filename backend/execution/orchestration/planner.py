...
class RoomSupervisorPlannerAdapter:
    ...
    async def _call_supervisor_service(
        ...
        system_prompt = (
            ...
            "You may coordinate specialist Agents as optional external tools. "
            ...
            "delegation. Always prefer a suitable delegation when the user explicitly "
            "asks an Agent to perform work or approves a previously offered Agent "
            "action. Suitability must be supported by the Agent Card's description "
            "or capabilities. Input-mode compatibility (for example, accepting text) "
            ...
            "When the user asks HYBRO to read, explain, or summarize an "
            "attachment and a ready text projection is listed in available_resources, "
            "return platform_answer. Do not delegate merely because a specialist Agent "
            "could perform a larger downstream workflow. The synthesis instruction "
            "must first answer from the attachment. When exactly one suitable Agent "
            ...
            "until the user confirms. If the user's next message selects or requests "
            "the offered result, treat that as approval of the Agent action.\n\n"
            ...
            "instead of copying resource contents into task.\n\n"
            "For a delegate action, each target may include context_refs, "
            "artifact_refs, attachment_refs, expected_outputs, and "
            "required_resource_refs. Select the smallest sufficient resource set by "
            "business relevance. Prefer a structured artifact over copied prose and "
