...
class AgentService:
    ...
    async def validate_agent_card(self, card_data: dict[str, Any]) -> list[str]:
        return await validate_agent_card_data(card_data)
    ...
    def _mask_sensitive_information(
        ...
        for field_path in fields:
            _mask_field_path(data, field_path)
        return AgentCenterResponse(**data)


def _mask_field_path(data: dict[str, Any], field_path: str) -> None:
    ...
    if "agent" in data:
        _remove_nested_field(data["agent"], parts)


def _remove_nested_field(obj: Any, path_parts: list[str]) -> None:
    if not path_parts:
        return
    if isinstance(obj, dict):
        _remove_nested_field_from_dict(obj, path_parts)
        return
    if isinstance(obj, list):
        for item in obj:
            _remove_nested_field(item, path_parts)


def _remove_nested_field_from_dict(obj: dict[str, Any], path_parts: list[str]) -> None:
    if len(path_parts) == 1:
        obj[path_parts[0]] = ""
    elif path_parts[0] in obj:
        _remove_nested_field(obj[path_parts[0]], path_parts[1:])
