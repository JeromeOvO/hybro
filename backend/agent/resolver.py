...
class AgentResolverService:
    ...
    def __init__(
        self,
        repository: AgentResolutionRepository | None = None,
        *,
        capability_issue_reader: Any | None = None,
        agent_selection_service: Any | None = None,
    ) -> None:
        self._resolution_repository = repository
        self.capability_issue_reader = capability_issue_reader
    ...
    def bind_agent_selection_service(self, service: Any) -> None:
        self.agent_selection_service = service

    def bind_capability_issue_reader(self, reader: Any) -> None:
        self.capability_issue_reader = reader
    ...
    async def resolve(
        ...
        *,
        ...
        if self.capability_issue_reader is not None:
            excluded = await self.capability_issue_reader.get_excluded_agent_ids()
        else:
            excluded = frozenset()
...
class AgentResolverFacadeRepository:
    ...
    async def query_similar_agents(
        ...
        if excluded_agent_ids:
            agents = [
                agent
                for agent in agents
                if agent is not None and agent.agent_id not in excluded_agent_ids
            ]
    ...
    async def get_agents_with_conditions_visible(
        ...
        infos = await self._facade.list_visible_agents(
            user_id=user_id,
            active_only=False,
            query=query,
            limit=limit,
