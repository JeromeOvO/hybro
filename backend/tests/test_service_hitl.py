...
class TestRequestInput:
    ...
    async def test_reused_agent_hitl_backfills_resolved_client_request_id(
        ...
        repair_call = mock_hitl_db_service.cas_update_hitl_request.await_args
        assert repair_call.args == ("hitl-reused",)
        assert repair_call.kwargs["expected_status"] == HITLStatus.PENDING.value
        assert repair_call.kwargs["client_request_id"] == "cr-reused-hitl"
        assert repair_call.kwargs["agent_prompt_hash"]
        event = mock_hitl_delivery.emit.await_args.args[0]
    ...
    async def test_blocking_followup_hitl_does_not_complete_new_pending_display_message(
        ...
        assert result["status"] == "ok"
        assert result["request_id"] == "hitl-old"
...
class TestGetPendingRequests:
    ...
    async def test_sanitizes_legacy_agent_requests_but_preserves_supervisor_prompts(
        ...
        requests = await hitl_service.get_pending_requests("room-123")

        assert len(requests) == 1
        supervisor_request = requests[0]
...
class TestHandleResponseErrors:
    ...
    @pytest.mark.asyncio
    async def test_supervisor_response_defers_orchestration_state_to_facade(
        self,
        hitl_service,
        mock_hitl_db_service,
    ):
        ...
        assert persisted.pending_hitl_request_ids == ["hitl-supervisor-1"]
        assert persisted.open_questions[0]["status"] == "open"
        assert persisted.facts == []
        hitl_service._continuation.resume_queue_from_continuation.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_handle_response_returns_orchestration_metadata_for_facade(
        self,
        hitl_service,
        mock_hitl_db_service,
        mock_hitl_delivery,
        ...
        assert result["status"] == "ok"
        assert result["request_id"] == request.request_id
        assert result["orchestration_run_id"] == "run-msg-1"
        assert result["response"] == "Account A"
        persisted = await run_store.get_run("run-msg-1")
        assert persisted is not None
        assert persisted.status == OrchestrationStatus.AWAITING_USER
        assert persisted.open_questions[0]["status"] == "open"
    ...
    def test_reducer_resolves_only_current_supervisor_request(self):
        state = OrchestrationRunState(
            run_id="run-msg-1",
            room_id="room-123",
            user_message_id="user-msg-456",
        ...
        assert updated.status == OrchestrationStatus.AWAITING_USER
        assert updated.pending_hitl_request_ids == ["hitl-1"]
        assert [question["status"] for question in updated.open_questions] == [
            "open",
            "resolved",
        ]
        assert updated.facts[0]["request_ids"] == ["hitl-2"]

    def test_recovery_retains_creating_and_cleanup_failed_pending_refs(self):
        state = OrchestrationRunState(
            run_id="run-msg-1",
            room_id="room-123",
            user_message_id="user-msg-456",
        ...
        assert updated.status == OrchestrationStatus.AWAITING_USER
        assert updated.pending_hitl_request_ids == ["hitl-2", "hitl-3"]
