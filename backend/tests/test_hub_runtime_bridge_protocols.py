...
def test_common_hub_protocol_shapes_are_split_between_async_liveness_and_sync_cache() -> (
    None
):
    ...
    assert (
        inspect.signature(HubManagement.connect_hub_stream).return_annotation
        != inspect.Signature.empty
    )
    assert inspect.iscoroutinefunction(HubDispatchPolicy.can_dispatch_to_hub)
    assert inspect.iscoroutinefunction(
        HubInternalResponseDispatcher.dispatch_hub_internal_response
    )
...
async def test_journal_accepts_naive_claim_datetimes() -> None:
    ...
    journal._records[active["journal_id"]]["claim_expires_at"] = datetime(2999, 1, 1)
...
@pytest.mark.asyncio
async def test_ownership_store_accepts_missing_hub_task_then_rejects_conflicting_aliases() -> (
    None
):
    ...
    second = await store.claim_or_refresh(
        {"hub_task_id": "hub-task"}, "worker-1", "lease-2"
    )
...
async def test_mongo_ownership_takeover_requires_atomic_expiry_match() -> None:
    ...
    class Collection:
        ...
        async def update_one(self, query, update, **kwargs):
            ...
            assert any(
                "lease_expires_at" in clause and "$lte" in clause["lease_expires_at"]
...
async def test_hub_mongo_repository_treats_matched_noop_update_as_success() -> None:
    ...
    assert await repo.update_hub_status_if_current("hub-1", "conn-1", is_online=False)
