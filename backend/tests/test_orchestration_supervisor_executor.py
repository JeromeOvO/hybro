...
async def test_dispatch_payload_failure_does_not_call_agent_processor():
    ...
    executor.orchestration_resource_provider.list_resources = AsyncMock(return_value=[])
...
async def test_run_materializes_only_selected_resource_refs_for_dispatch():
    ...
    assert (
        dispatched_message.extend_info["public_dispatch_text"]
...
async def test_agent_result_records_same_shadow_observations_regardless_of_guardrails(
    ...
    async def run_case(guardrails_enabled: bool) -> dict[str, object]:
        ...
        await executor._ingest_orchestration_results(
            state=state,
            results=[result],
        ...
        comparable = {
            ...
            "blockers": [blocker.model_dump(mode="json") for blocker in saved.blockers],
            "continuations": [
...
async def test_platform_answers_directly_when_no_candidate_agent_is_suitable():
    user_message = RoomUserMessage(
        ...
        message_id="message-1",
        user_id="user-1",
        message_content=MessageContent(message_text="What is 2 + 2?"),
        extend_info={
            "orchestration": True,
            "orchestration_run_id": "message-1",
            "candidate_agent_ids": [],
    ...
    planner = RecordingPlanner(
        PlannerAction(
            ...
            reasoning="No connected agent is suitable for this general question.",
            synthesis_instruction="Answer the question directly.",
        )
    )
    store = InMemoryOrchestrationRunStore()
    executor = _executor(store=store, planner=planner, user_message=user_message)
    ...
    executor.agent_message_processor.process_single_message.assert_not_awaited()
    synthesis_instruction = executor._stream_supervisor_synthesis.await_args.kwargs[
...
async def test_run_delegate_path_publishes_exact_dispatched_task(
    ...
    assert processor_call.kwargs["attachment_forwarding_policy"] == ("compatible_only")
...
async def test_attachment_preflight_failure_update_uses_public_task_label(
    ...
    persisted_message = executor.message_writer.add_room_agent_message.await_args.args[
        0
    ]
...
def test_orchestration_dispatch_intent_preserves_dispatch_metadata():
    refs = _dispatch_refs_payload()

    intent = SupervisorExecutor._orchestration_dispatch_intent(
        run_id="run-1",
        step_number=2,
        target_index=3,
...
def test_orchestration_supervisor_action_preserves_dependency_metadata():
    planner_action = PlannerAction(
        action=PlannerActionType.DELEGATE,
    ...
    action = SupervisorExecutor._orchestration_supervisor_action(
        planner_action,
        [AgentProfile(agent_id="agent-1", agent_name="Agent One")],
    )
...
async def test_ingest_agent_results_serially_ignores_event_append_failures(monkeypatch):
    ...
    monkeypatch.setattr(
        store, "append_event", AsyncMock(side_effect=RuntimeError("oops"))
    )
...
@pytest.mark.asyncio
async def test_orchestration_results_are_ingested_with_state_writes_per_result(
    monkeypatch,
):
    writes: list[int] = []
    ...
    state.dispatch_intents = [
        executor._orchestration_dispatch_intent(
            run_id="run-1",
            step_number=1,
            target_index=1,
            target=DelegateTarget(
    ...
    updated = await executor._ingest_orchestration_results(
    ...
    outputs_by_id = {
        output.agent_message_id: output for output in updated.agent_outputs
    }
    assert outputs_by_id["run-1:step-1:target-1:message"].status == ("completed")
    assert outputs_by_id["run-1:step-1:target-2:message"].status == "failed"


@pytest.mark.asyncio
async def test_orchestration_result_without_message_id_updates_only_one_fallback_intent():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
...
@pytest.mark.asyncio
async def test_orchestration_result_without_message_id_updates_fallback_intents_in_order():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
    ...
    assert updated.dispatch_intents[1].status == StepStatus.FAILED.value
    assert updated.state_version == 2
    output_messages = [output.agent_message_id for output in updated.agent_outputs]
...
async def test_run_ask_user_creates_hitl_prompt_and_continuation():
    ...
    executor._checkpoint_interrupt = AsyncMock(side_effect=save_interrupted_state)
...
async def test_run_resumes_ingesting_hitl_without_replanning_or_duplicates():
    ...
    state.steps_used = 1
    state.pending_hitl_request_ids = ["run-message-1:step-1:supervisor-hitl-1"]
    state.decision_log = [
        {
...
async def test_run_ask_user_cleanup_on_final_state_save_failure(monkeypatch):
    ...
    assert persisted.terminal_summary["reason"] == (
        "failed to persist orchestration supervisor HITL state"
    )
...
async def test_run_ask_user_records_creating_question_before_request_input():
    ...
    assert any(
        question.get("request_id") == "hitl-1" and question.get("status") == "open"
...
async def test_run_ask_user_records_failed_message_cleanup_without_pending_request():
    ...
    cleanup_question = next(
        question
        for question in persisted.open_questions
        if question.get("request_id") == "message-1:step-1:supervisor-hitl-1"
    )
    ...
    assert any(
        ...
        and "message-1:step-1:supervisor-hitl-1:message" in fact.get("message_ids", [])
...
@pytest.mark.asyncio
async def test_run_agent_awaiting_input_checkpoint_interrupt_exception_cancels_and_fails():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_run_ask_user_checkpoint_interrupt_exception_clears_transient_state():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
async def test_run_mixed_paused_and_awaiting_input_creates_hitl_prompt():
    ...
    save_kinds = [
        ...
        for call in executor._checkpoint_interrupt.await_args_list
    ]
...
async def test_run_multiple_awaiting_input_results_keep_secondary_awaiting_input_recoverable():
    ...
    (
        recovered_state,
        recover_status,
    ) = await recover_executor._recover_orchestration_inflight_dispatch(
        state=state,
        agent_registry=[
            AgentProfile(agent_id="agent-1", agent_name="Agent One"),
            AgentProfile(agent_id="agent-2", agent_name="Agent Two"),
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_rehydrates_awaiting_input_output_to_hitl():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
    ...
    (
        recovered_state,
        run_status,
    ) = await executor._recover_orchestration_inflight_dispatch(
        state=state,
        agent_registry=[AgentProfile(agent_id="agent-1", agent_name="Agent One")],
        room_config=RoomConfig(),
        room_id="room-1",
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_ingests_plain_a2a_input_required_for_planner():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_rehydrates_a2a_task_fields_for_auth_required():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_rehydrates_policy_required_task_state():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_paused_and_awaiting_saves_paused_before_hitl():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_paused_and_awaiting_fails_without_hitl_if_save_fails():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_surfaces_outcome_event_append_failures(
    monkeypatch,
):
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_retries_missing_persisted_outcome_event(
    monkeypatch,
):
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_persists_one_idempotent_outcome():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_validates_agent_missing_fields_as_blocker():
    store = InMemoryOrchestrationRunStore()
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
    ...
    executor._orchestration_artifacts_for_output_message = AsyncMock(
        return_value=[
            {
                "artifact_id": "submission",
                "name": "submission",
...
async def test_resource_backed_outcome_blocks_matching_delegate_revision():
    ...
    persisted = await executor._ingest_orchestration_results(
        state,
        [
            StepResult(
                step_number=1,
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_segments_outcomes_by_selected_resource_content():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
...
@pytest.mark.asyncio
async def test_append_orchestration_event_swallows_non_required_store_conflicts(
    monkeypatch,
):
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
    ...
    await executor._append_orchestration_event(
        _run_state(),
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_persists_structured_validated_blockers():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
...
async def test_goal_family_disposition_terminalizes_related_work_and_records_event():
    ...
    saved = await executor._dispose_orchestration_goal_family(
        await store.get_run("run-1"),
        goal_family_fingerprint="family-1",
...
async def test_goal_family_disposition_requires_nonempty_reason():
    ...
    with pytest.raises(ValueError, match="reason"):
        await executor._dispose_orchestration_goal_family(
            await store.get_run("run-1"),
            goal_family_fingerprint="family-1",
            through_goal_revision_fingerprint="revision-1",
            status="abandoned",
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_distinguishes_changed_error_message():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
    ...
    state.dispatch_intents = [
        executor._orchestration_dispatch_intent(
            ...
            target_index=1,
            target=DelegateTarget(
                agent_id="agent-1", agent_name="Agent One", task="one"
            ),
        )
    ]
    await store.create_run(state)
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_distinguishes_changed_artifact_content():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
...
@pytest.mark.asyncio
async def test_recover_orchestration_inflight_dispatch_preserves_artifacts_for_replayed_output():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_run_accepts_provider_done_alias_from_room_supervisor_adapter():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
    ...
    legacy_actions = [
        ...
        {
            "action": "done",
            "reasoning": "provider done alias after agent response",
        },
    ]
    ...
    assert (
        result.run_state.terminal_reason == "provider done alias after agent response"
    )
    assert result.run_state.decision_log[-1]["action"] == "complete"


@pytest.mark.asyncio
async def test_run_accepts_provider_done_alias_with_facts_only_state():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
...
@pytest.mark.asyncio
async def test_blocking_agent_hitl_terminal_message_replaces_stale_awaiting_output():
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
        user_id="user-1",
    ...
    executor.message_reader.get_room_agent_message_by_message_id = AsyncMock(
        side_effect=lambda message_id: (
            SimpleNamespace(
                ...
                last_notified_state="completed",
                message_content=SimpleNamespace(
                    message_text="Completed after blocking HITL",
                    message_task=SimpleNamespace(
    ...
    assert result.status == RunStatus.COMPLETED
    recovered = await store.get_run("message-1")
    assert recovered is not None
    assert recovered.agent_outputs[0].status == "completed"
    assert recovered.agent_outputs[0].text == "Completed after blocking HITL"
    assert planner.contexts[0].state_context.agent_outputs[0]["status"] == "completed"
...
@pytest.mark.asyncio
async def test_durable_envelope_routes_to_single_orchestration_path():
...
def test_orchestration_envelope_requires_candidate_scope():
    assert RoomMessageCenter._is_supervisor_envelope({}) is False
    assert (
        RoomMessageCenter._is_supervisor_envelope(
            {
                "orchestration": True,
                "orchestration_run_id": "message-1",
                "candidate_agent_ids": ["agent-1"],
    ...
    assert (
        RoomMessageCenter._is_supervisor_envelope(
            {
                "orchestration": True,
                "orchestration_run_id": "message-2",
                "candidate_agent_ids": [],
            }
        )
        is True
...
@pytest.mark.asyncio
async def test_agent_input_required_ignores_unrelated_projection():
    store = InMemoryOrchestrationRunStore()
    user_message = RoomUserMessage(
        room_id="room-1",
        user_id="user-1",
            ...
            facts=[
                {
                    "fact_id": "ctx:file-file-1:text",
                    "kind": "attachment_projection",
                    "text": "Warehouse inspection notes only.",
                }
            ],
            agent_outputs=[
                AgentOutputRecord(
...
async def test_agent_input_required_does_not_replay_original_dispatch_refs():
    ...
    context_ref = DispatchContentRef(
        kind=DispatchRefKind.CONTEXT,
        ref_id="ctx:file-file-1:text",
        mime_type="text/plain",
    )
    ...
    executor.orchestration_resource_provider = SimpleNamespace(
        resolve_ref=AsyncMock(
            return_value=ResolvedResourcePayload(
                ...
                kind="context",
                mime_type="text/plain",
                text="Projected submission from resource provider.",
            )
        )
    )
    reply_to_task = AsyncMock(
        return_value={
            "blocking": True,
            "task_state": "completed",
            "response_text": "Continued with provider payload",
        }
    )
    executor.hitl_coordinator = SimpleNamespace(
        request_input=AsyncMock(
...
async def test_agent_input_required_nested_reply_remains_awaiting_input():
    ...
    _, [resumed], follow_up_ids = await executor._resolve_agent_input_required_results(
        state=state,
        results=[awaiting],
        user_message=user_message,
    )
...
@pytest.mark.asyncio
async def test_hitl_answer_follow_up_input_required_stays_awaiting_input():
    store = InMemoryOrchestrationRunStore()
    user_message = RoomUserMessage(
        room_id="room-1",
        user_id="user-1",
        message_content=MessageContent(message_text="$2M"),
        message_id="msg-2",
    )
        ...
        agent_reply=SimpleNamespace(
            reply_to_task=AsyncMock(
                return_value={
                    "blocking": True,
                    "task_state": "input-required",
                    "response_text": "Need coverage limit too.",
                }
            )
        )
    )
...
def test_awaiting_result_requires_hitl_only_for_auth_or_policy():
    ...
    assert (
        SupervisorExecutor._awaiting_result_requires_hitl(plain_input_required) is False
    )
...
def test_resolved_resource_fingerprints_prefer_private_catalog_fingerprint():
    ...
    assert SupervisorExecutor._resolved_resource_fingerprints(resolved_payload) == {
...
async def test_concurrent_delegate_recovery_claims_before_one_remote_reply():
    ...
    await asyncio.gather(
        executor._dispatch_targets(
            ...
            request_user_id="user-1",
            quoted_text=None,
            planned_message_ids=["repair-msg"],
            run_state=state,
...
async def test_exceptional_continuation_reply_reopens_persisted_claim():
    ...
    executor.hitl_coordinator = SimpleNamespace(
        agent_reply=SimpleNamespace(
            reply_to_task=AsyncMock(side_effect=RuntimeError("remote unavailable"))
        )
    )
    ...
    with pytest.raises(RuntimeError, match="remote unavailable"):
        await executor._continue_agent_task_with_resolved_refs(
            ...
            target=DelegateTarget(
                agent_id="agent-1", agent_name="Agent One", task="retry"
            ),
            resolved_payload=ResolvedDispatchPayload(
                resource_payloads=[
                    ResolvedResourcePayload(
...
@pytest.mark.asyncio
async def test_ingest_orchestration_results_reopens_repeated_plain_input_and_resolves_continuation():
    store = InMemoryOrchestrationRunStore()
    executor = _executor(
        store=store,
        planner=RecordingPlanner(),
    ...
    current = await executor._ingest_orchestration_results(
    ...
    current = await executor._claim_matching_continuation(
        ...
        goal_family_fingerprint=current.pending_agent_continuations[
            0
        ].goal_family_fingerprint,
        selected_resource_fingerprints={"resource-new"},
    )
    ...
    current = await executor._ingest_orchestration_results(
        await store.get_run("run-1"),
        [
            plain_input.model_copy(
                update={"status": StepStatus.SUCCESS, "success": True}
            )
        ],
        status=OrchestrationStatus.RUNNING,
        advance_step=False,
...
async def test_plain_continuation_preserves_auth_required_classification():
    ...
    assert SupervisorExecutor._awaiting_result_requires_hitl(result) is True
    assert (
        SupervisorExecutor._is_plain_a2a_input_output(
            AgentOutputRecord(
    ...
    state = _run_state(
        dispatch_intents=[
            ...
            DispatchIntent(
                ...
                step_target_id="target-2",
                dispatch_intent_id="intent-2",
                planned_agent_message_id="agent-msg-1-repair",
                agent_id="agent-1",
                task="Repair",
                task_hash="hash-2",
                repair_of_intent_id="intent-1",
        ...
        active_dispatches=[
            ...
            {
                "agent_message_id": "agent-msg-1-repair",
                "agent_id": "agent-1",
                "status": "dispatching",
            },
        ],
...
async def test_delegate_recovery_reuses_message_and_closes_current_intent():
    ...
    executor.orchestration_resource_provider = SimpleNamespace(
        resolve_ref=AsyncMock(return_value=resource_payload)
    )
...
async def test_pdf_capable_upstream_gets_projection_and_source_attachment_before_downstream_dispatch():
    ...
    executor._orchestration_dispatch_intent = MagicMock(side_effect=dispatch_intent)
...
@pytest.mark.asyncio
async def test_mark_orchestration_terminal_persists_actionable_terminal_summary():
    store = InMemoryOrchestrationRunStore()
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
    ...
    updated = await executor._mark_orchestration_terminal(
        state,
    ...
    assert (
        terminal_event.payload["terminal_summary"]["recommended_next_action"]
        == "ask_user"
    )
...
@pytest.mark.asyncio
async def test_mark_orchestration_terminal_omits_failure_summary_for_completed_status():
    store = InMemoryOrchestrationRunStore()
    user_message = RoomUserMessage(
        room_id="room-1",
        message_id="message-1",
...
@pytest.mark.asyncio
async def test_mark_orchestration_terminal_clears_summary_for_canceled_status():
    store = InMemoryOrchestrationRunStore()
    ...
    assert updated.terminal_summary is None
    assert result.terminal_summary is None
    terminal_event = store._events_by_run[updated.run_id][-1]
    assert terminal_event.event_id == (
        f"{updated.run_id}:run-terminal:canceled:{updated.state_version}"
    )
    assert "terminal_summary" not in terminal_event.payload
...
@pytest.mark.asyncio
async def test_interrupt_checkpoint_does_not_serialize_supervisor_continuation():
    executor = object.__new__(SupervisorExecutor)
    trajectory = SupervisorTrajectory()
    paused = StepResult(
        step_number=1,
        agent_id="agent-1",
        ...
        paused_message_id="agent-message-1",
    )

    checkpointed = await executor._checkpoint_interrupt(
        InterruptKind.PUSH_NOTIFICATION,
        trajectory=trajectory,
        room_id="room-1",
        user_message_id="message-1",
        message_text="coordinate",
        agent_registry=[],
        room_config=RoomConfig(),
        conversation_context=None,
        request_user_id="user-1",
