"""Guarded remote fetch and Room-owned artifact storage adapters."""
    ...
    async def commit(
        ...
        *,
        ...
        try:
            ...
            # must stay stable across version-neutral naming cleanups
...
class GuardedRoomFileArtifactWriter:
    ...
    async def __call__(
        ...
        if len(content) > self.max_bytes:
            raise ValueError("remote artifact exceeds the configured owner limit")
        ...
        # identity of every artifact already written by earlier deployments and
        ...
        # a version-neutral naming cleanup.
        origin_key = sha256(
            "|".join(
                (
                    "orchestrator-v3-a2a",
