...
Backend queue/resume completion paths set `turn_completion_kind` from `_emit_unified_summary` return value (`synthesis` when LLM summary emitted, `deterministic` when fewer than two agents respond). Terminal `processing_status` details include `turn_phase: 'terminal'`; synthesis stage emits `turn_phase: 'synthesizing'`. The frontend stores `turnPhase` on `ProcessingStatusLogEntry` when appending logs from SSE `details`.
