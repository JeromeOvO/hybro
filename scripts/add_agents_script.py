...
framework_agents = {
    "crewai": {
        ...
        "provider": {"organization": "CrewAI Integration", "url": "https://crewai.io"},
        "capabilities": {
            "streaming": False,
...
def create_agent(agent_card: dict[str, Any]) -> dict[str, Any] | None:
    ...
    payload = {
        ...
        "agentCard": {
            ...
            "provider": agent_card.get(
                "provider",
                {"organization": "Default Organization", "url": "https://example.com"},
            ),
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            print(
                f"❌ Failed to create agent {agent_card['name']}: {response.status_code} - {response.text}"
            )
