...
Usage:
  python scripts/migrate_room_memories.py               (dry run)
  python scripts/migrate_room_memories.py --execute     (apply all)
  python scripts/migrate_room_memories.py --room-id <ROOM_ID>            (dry run room)
  python scripts/migrate_room_memories.py --room-id <ROOM_ID> --execute  (apply room)
...
def migrate_turn(turn: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from models.memory import ContentType, TurnRepresentation, TurnType

    # Skip if already migrated (has turn_id)
    if turn.get("turn_id"):
        return turn

    content = turn.get("content", "")

    # Add new fields (use enum values for consistency)
    turn["turn_id"] = str(uuid4())
    turn["representation"] = TurnRepresentation.FULL.value
    turn["content_type"] = ContentType.TEXT.value
    turn["turn_type"] = TurnType.MESSAGE.value
    turn["estimated_tokens_full"] = estimate_tokens_simple(content)
    turn["estimated_tokens_compact"] = 20  # Default for compact representation
    turn["turn_notes"] = extract_turn_notes_simple(content)
    turn["content_ref"] = None
    turn["brief_summary"] = None
    turn["was_successful"] = None  # Unknown for legacy turns
...
def migrate_room_memory(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    # Add room_facts if missing
    if not doc.get("room_facts"):
        updates["room_facts"] = []

    # Add agent_success_history if missing
    ...
    if not doc.get("last_activity_at"):
        # Use memory_created_at as fallback
        updates["last_activity_at"] = doc.get("memory_created_at", datetime.utcnow().isoformat())
...
async def migrate_room_memories(room_id: str | None, dry_run: bool) -> None:
    ...
    room_memories = mongodb.room_memories_collection
    ...
    for doc in docs:
        try:
            ...
            if dry_run:
                print(
                    f"[DRY-RUN] room_id={doc.get('room_id')} would update {len(updates)} fields: "
                    f"{list(updates.keys())}"
                )
            else:
                await room_memories.update_one({"_id": doc["_id"]}, {"$set": updates})
...
if __name__ == "__main__":
    ...
    asyncio.run(migrate_room_memories(room_id=args.room_id, dry_run=not args.execute))
