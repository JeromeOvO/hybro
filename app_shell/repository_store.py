...
class AppShellRepositoryStore:
    ...
    async def get_task_messages_for_room(
        ...
        return [
            msg
            for msg in (_safe_parse_agent_message(doc) for doc in docs if doc is not None)
            if msg is not None
        ]
    ...
    async def iter_stale_processing_hitl_requests(
        ...
        try:
            ...
            logger.error("Failed to iterate stale processing HITL requests", exc_info=True)
            docs = []
        for doc in docs:
            yield doc
    ...
    async def update_task_state_on_message(
        ...
        *,
        ...
        try:
            ...
            updated = await self._message_repository.update_agent_message_if_not_terminal(
                message_id,
                updates,
                terminal_values,
            )
