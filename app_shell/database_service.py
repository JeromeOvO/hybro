...
class AgentGroupStore(Protocol):
    ...
    async def delete_agent_group(self, group_id: str, owner_id: str) -> bool: ...
    ...
    async def update_agent_group(
        self, group_id: str, owner_id: str, data: dict[str, str | list[str]]
    ) -> bool: ...
...
class DatabaseService:
    ...
    async def claim_hitl_group_routing(self, group_id: str, claim_id: str) -> bool:
        """Claim responsibility for routing a completed HITL group once."""
        try:
            result = await self.mongo.db.hitl_requests.update_one(
                {
                    "group_id": group_id,
