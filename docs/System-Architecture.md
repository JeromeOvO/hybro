...
**Conversation markdown normalization** (`src/lib/markdown/normalize-conversation.ts` + `remark-conversation-lists.ts`, applied in `MarkdownContent` when `className` includes `conversation-markdown-body`):

- Pre-parse inline ordered split (`split-inline-ordered.ts`) handles run-on `1. foo 2. bar` lines that remark cannot recover after parse. Heading lines (ATX `#…`) and fenced code are skipped so `#### 1. **Title**` is not split into an empty heading + list. Bare ATX markers on their own line (`###\n1. **Title**`) are folded into the next content line so agents that emit the heading marker separately from the title do not produce empty `<h3>` + single-item `<ol>` pairs.
- Full normalization uses **unified/remark**: `remark-parse` → `remark-gfm` → custom `remarkConversationLists` → `remark-stringify`. Agent `message_text` is stored and returned by the backend as produced by the agent; markdown repair is client-side only before Streamdown render.
- `remarkConversationLists` is intentionally minimal (no prose-pattern heuristics). It does two passes over root blocks:
  1. **Adjacent bullet nesting** — a `<ul>` that is a direct sibling of an `<ol>` (i.e. no heading, paragraph, or thematic break between them) is moved into the last `<li>` of that `<ol>`. Any intervening prose terminates the previous ordered section automatically because the bullet list is no longer adjacent.
  2. **Sequential `start` assignment** — each top-level `<ol>` gets a `start` value so repeated `1.` markers from agents render as `1, 2, 3 …` across consecutive lists. The counter resets to 1 after any heading, thematic break, or non-empty plain paragraph.
- The renderer (`MarkdownContent`) maps top-level `<ol>` elements to set `style.counterReset = 'conv-section-ol <start - 1>'` from the `start` prop (nested `<ol>` elements rely on CSS `conv-nested-ol` only). Each top-level `<ol>` is self-describing so a single source markdown list cannot drift even if Streamdown splits it into multiple DOM `<ol>` chunks. CSS counters in `conversation-tokens.css` then provide the visible `N.` markers.
