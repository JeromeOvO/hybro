...
class DebateService:
    ...
    async def _conduct_debate_round(self, debate_session: Dict[str, Any], round_num: int):
        ...
        for i, response in enumerate(responses):
            ...
            if isinstance(response, Exception):
                print(f"Error getting response from agent {agent_info['agent_id']}: {response}")
                response_text = f"Agent {agent_info['agent_id']} failed to respond: {str(response)}"
            else:
    ...
    async def _get_agent_response(
        ...
        try:
            ...
            context = self._prepare_debate_context(debate_history, agent_info["agent_id"])
            
            # Create the debate prompt
            if round_num == 1:
                prompt = f"Please provide your analysis and opinion on: {original_input}"
            else:
...
Based on the discussion so far, please provide your updated analysis and opinion. You can agree, disagree, or build upon previous points."""
            ...
            if agent_info.get("is_remote", False):
                ...
                if round_num == 1:
                    return await self.openai_service.short_debate_with_openai(original_input, "")
                else:
                    return await self.openai_service.long_debate_with_openai(original_input, context)
    ...
    def _prepare_debate_context(self, debate_history: List[Dict[str, Any]], current_agent_id: str) -> str:
        """Prepare context from debate history for an agent."""
        
        context_parts = []
        for entry in debate_history:
            if entry["agent_id"] != current_agent_id:  # Exclude current agent's own responses
    ...
    async def _get_agent_confirmations(
        ...
        confirmations = await asyncio.gather(*confirmation_tasks, return_exceptions=True)
        
        # Count positive confirmations
        confirmation_count = 0
        for i, confirmation in enumerate(confirmations):
            if isinstance(confirmation, Exception):
                print(f"Error getting confirmation from agent {debate_session['agents'][i]['agent_id']}: {confirmation}")
                continue
...
Do you agree with this final conclusion? Please respond with "Yes, I agree" or "No, I disagree" followed by a brief explanation."""
            ...
            print(f"Error getting confirmation from agent {agent_info['agent_id']}: {e}")
    ...
    def _is_positive_confirmation(self, response: str) -> bool:
        ...
        positive_indicators = [
            "yes", "agree", "correct", "accurate", "right", 
            "confirm", "support", "endorse", "accept"
        ]
        negative_indicators = [
            "no", "disagree", "incorrect", "wrong", "reject", 
            "oppose", "deny", "refuse"
        ]
