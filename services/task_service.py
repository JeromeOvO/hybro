...
class TaskService:
    ...
    async def create_child_task(self, root_task_id: str, description: str, priority: Optional[int] = None, dependencies: Optional[List[str]] = None) -> str:
        """
        ...
        child_base_task = Task(
            id=None,
            sessionId=None,  # Will inherit from parent
            status=TaskStatus(
                state=TaskState.SUBMITTED,
                timestamp=datetime.now()
            ),
        ...
        child_task = ChildTask(
            task_id=None,  # Will be generated by the database service
        ...
        child_task.task.sessionId = root_task.task.sessionId
