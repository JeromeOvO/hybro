...
def _multi_event_sequence(*, skip_persist: bool) -> list[AgentEvent]:
    """token -> token -> artifact_update -> response (terminal)."""
    return [
        AgentEvent(kind="token", **_base(), text="Hello ", skip_persist=skip_persist),
        AgentEvent(kind="token", **_base(), text="world", skip_persist=skip_persist),
        AgentEvent(
