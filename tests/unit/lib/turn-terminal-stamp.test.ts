...
describe('canStampTurnTerminalFromEntityState', () => {
  it('returns false for multi-agent turn without summary during pre-synthesis gap', () => {
    const turn = makeTurn({
  ...
  it('returns false when backend confirms done during pre-synthesis gap', () => {
    const turn = makeTurn({
      status: 'active',
      agentResults: [
        makeAgent({ messageId: 'a1', agentId: 'agent-a' }),
    ...
    expect(canStampTurnTerminalFromEntityState(turn, undefined, false, { lifecycleActive: true })).toBe(false)
  })
  ...
  it('returns true for LLM summary with streamed content while still working', () => {
    const turn = makeTurn({
      agentResults: [
        makeAgent({ messageId: 'a1', agentId: 'agent-a' }),
        makeAgent({ messageId: 'a2', agentId: 'agent-b' }),
...
describe('ensureTurnTerminalStampedFromBackendTruth', () => {
  ...
  it('does not stamp when inquiryActiveRuns is empty during pre-synthesis gap', async () => {
