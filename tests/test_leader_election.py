"""Tests for app_shell.redis_runtime module.
...
def _make_redis(set_nx_return=True, eval_return=1):
    """Helper to create a mock AppShellRedisService."""
    redis = MagicMock()
...
class TestLeaderAcquire:
    ...
    async def test_acquire_succeeds_first_time(self):
        ...
        leader = AppShellLeaderElection(_make_redis(set_nx_return=True), instance_id="inst-A")
    ...
    async def test_acquire_fails_when_held(self):
        ...
        leader = AppShellLeaderElection(_make_redis(set_nx_return=False), instance_id="inst-B")
    ...
    async def test_acquire_passes_correct_key_and_ttl(self):
        ...
        leader = AppShellLeaderElection(redis, instance_id="inst-A")
...
class TestLeaderRenew:
    ...
    async def test_renew_succeeds_for_current_leader(self):
        ...
        leader = AppShellLeaderElection(_make_redis(eval_return=1), instance_id="inst-A")
    ...
    async def test_renew_fails_for_non_leader(self):
        ...
        leader = AppShellLeaderElection(_make_redis(eval_return=0), instance_id="inst-B")
...
class TestLeaderRelease:
    ...
    async def test_release_noop_for_non_leader(self):
        ...
        leader = AppShellLeaderElection(redis, instance_id="inst-B")
...
class TestLeaderHoldContext:
    ...
    async def test_hold_raises_when_not_acquired(self):
        ...
        leader = AppShellLeaderElection(redis, instance_id="inst-A")
        with pytest.raises(AppShellLeaderElection.NotAcquiredError):
