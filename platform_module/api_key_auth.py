...
from common.protocols import APIKeyPrincipal, APIKeyValidationStore
...
class MongoAPIKeyAuthenticator:
    ...
    async def validate_api_key(
        ...
        if not api_key_doc.is_active:
            logger.warning(
                "API key validation failed: key %s is inactive",
                api_key_doc.key_id,
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail={"error": "key_inactive", "message": "API key is inactive"},
            )
        ...
        if track_usage:
            try:
                ...
                logger.warning("Failed to update API key usage: %s", exc)
